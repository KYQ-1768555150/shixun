          ...
          <div class="table">
            <el-table :data="ordersData" strip>
              ...
              <el-table-column label="操作" align="center" width="180">
                <template v-slot="scope">
                  <el-button size="mini" type="primary" v-if="scope.row.status === '待收货'" plain @click="updateStatus(scope.row, '已完成');loadGoods()">确认收货</el-button>
...
export default {
  ...
  methods: {
    loadGoods() {
      this.$request.get('/goods/selectById?id=' + this.form.goodsId).then(res => {
        if (res.code === '200') {
    ...
    updateGoods() {
      this.goodsData.count+=this.form.num

      this.$request.put('/goods/update', this.goodsData).then(res => {
        if (res.code === '200') {
